...
export namespace SessionRevert {
  ...
  export async function cleanup(session: Session.Info) {
    ...
    for (const msg of remove) {
      await Database.use(async (db) => db.delete(MessageTable).where(eq(MessageTable.id, msg.info.id)).run())
    ...
    if (session.revert.partID && target) {
      ...
      if (removeStart >= 0) {
        ...
        for (const part of removeParts) {
          await Database.use(async (db) => db.delete(PartTable).where(eq(PartTable.id, part.id)).run())
