...
export namespace Workspace {
  ...
  export const get = fn(WorkspaceID.zod, async (id) => {
    const row = await Database.use(async (db) => db.select().from(WorkspaceTable).where(eq(WorkspaceTable.id, id)).get())
    if (!row) return
    return fromRow(row)
  })
  ...
  export const remove = fn(WorkspaceID.zod, async (id) => {
    ...
    if (row) {
      ...
      adaptor.remove(info)
      await Database.use(async (db) => db.delete(WorkspaceTable).where(eq(WorkspaceTable.id, id)).run())
      return info
    }
  })
